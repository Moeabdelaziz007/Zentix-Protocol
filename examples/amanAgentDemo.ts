...
async function runAmanAgentDemo(): Promise<void> {
  AgentLogger.log(LogLevel.INFO, 'AmanAgentDemo', 'Starting AmanAgent (Wasta-Bot) Demo');
  ...
  try {
    ...
    AgentLogger.log(LogLevel.SUCCESS, 'AmanAgentDemo', 'AmanAgent initialized');
    ...
    AgentLogger.log(LogLevel.INFO, 'AmanAgentDemo', 'Stored user credentials for ICP platform');
    ...
    AgentLogger.log(LogLevel.INFO, 'AmanAgentDemo', 'Created passport renewal task', { taskId: passportTask.id });
    ...
    if (processedTask.result?.success) {
      AgentLogger.log(LogLevel.SUCCESS, 'AmanAgentDemo', 'Passport renewal task completed successfully', {
        referenceNumber: processedTask.result.referenceNumber,
        nextSteps: processedTask.result.nextSteps
      });
    } else {
      AgentLogger.log(LogLevel.ERROR, 'AmanAgentDemo', 'Passport renewal task failed', {
    ...
    AgentLogger.log(LogLevel.INFO, 'AmanAgentDemo', 'Retrieved user service statuses', { count: serviceStatuses.length });
    ...
    if (expiringServices.length > 0) {
      AgentLogger.log(LogLevel.WARN, 'AmanAgentDemo', 'Found expiring services', { count: expiringServices.length });
    ...
    AgentLogger.log(LogLevel.INFO, 'AmanAgentDemo', 'Supported government services', { 
      count: supportedServices.length,
      services: supportedServices.map(s => s.name)
    });

    AgentLogger.log(LogLevel.SUCCESS, 'AmanAgentDemo', 'AmanAgent (Wasta-Bot) Demo completed successfully');
  } catch (error) {
    AgentLogger.log(LogLevel.ERROR, 'AmanAgentDemo', 'AmanAgent Demo failed', {}, error as Error);
    throw error;
  }
}
